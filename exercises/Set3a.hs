...
capitalize x = unwords . map capitalizeWord . words $ x
...
powers :: Int -> Int -> [Int]
powers k max = map (k^) . takeWhile (\x -> k^x <= max) $ [0..max]
...
whileRight f x
    | isRight (f x) = whileRight f . myFromRight . f $ x
    | otherwise     = myFromLeft (f x)
...
joinToLength len lst = [x ++ y | x <- lst, y <- lst, length (x ++ y) == len]
...
interpreter' [] _ _ prints = prints
interpreter' (c:commands) x y prints = case c of "up"     -> interpreter' commands x (y + 1) prints
                                                 "down"   -> interpreter' commands x (y - 1) prints
                                                 "left"   -> interpreter' commands (x - 1) y prints
                                                 "right"  -> interpreter' commands (x + 1) y prints
                                                 "printX" -> show x : interpreter' commands x y prints
                                                 "printY" -> show y : interpreter' commands x y prints
