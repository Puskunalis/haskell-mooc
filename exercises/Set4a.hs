...
longest (x:xs) = go x xs
    where go lst (x:xs)
            | length x == length lst && head x < head lst || length x > length lst = go x xs
            | otherwise                                                            = go lst xs
          go lst [] = lst
...
incrementKey key = map (\x -> if fst x == key then (fst x, 1 + snd x) else x)
...
average xs = go 0 0 xs
    where go sum n []     = sum / n
          go sum n (x:xs) = go (sum + x) (n + 1) xs
...
freqs xs = foldr (Map.alter go) Map.empty xs
    where go (Just x) = Just (x + 1)
          go Nothing  = Just 1
...
transfer from to amount bank =
    if amount > 0 && Map.member to bank && Map.findWithDefault 0 from bank >= amount
...
swap' i arr val = arr // [(i, val) | x <- [val]]

swap :: Ix i => i -> i -> Array i a -> Array i a
swap i j arr = swap' i (arr // [(j, arr ! x) | x <- [i]]) (arr ! j)
...
maxIndex arr = go (assocs arr) (fst . bounds $ arr)
    where go (x:xs) idx = if snd x > arr ! idx then go xs (fst x) else go xs idx
          go [] idx     = idx
