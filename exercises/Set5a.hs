...
up position = Position (x position) (y position + 1)

-- right increases the x value of a position by one
right :: Position -> Position
right position = Position (x position + 1) (y position)
...
rgb (Mix color1 color2) = map (/2) (sumArrays (rgb color1) (rgb color2))
...
toList (Pair k v keyVals) = (k,v):toList keyVals
toList _ = []
...
toNat :: Int -> Maybe Nat
toNat z = if z >= 0 then Just (go z) else Nothing
  where go z
          | z > 0     = PlusOne (go (z - 1))
          | otherwise = Zero
...
toBin 0 = O End
toBin 1 = I End
toBin n = if n `mod` 2 == 1 then I (toBin (n `div` 2)) else O (toBin (n `div` 2))
